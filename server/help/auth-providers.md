...
NyaitterServer では、複数のログイン方法（認証プロバイダー）を用途や好みに合わせて自由に有効化・設定できます。
...
| 認証プロバイダー | 説明 | 主な用途 |
|---|---|---|
| **1. Scratch認証** | Scratchのコメント欄を使った確実な本人確認 | Scratcher向けコミュニティ（デフォルト） |
| **2. メールアドレス認証** | 6桁の認証コード（OTP）による安全なログイン | 一般ユーザー・Scratch未利用者向け |
...
### パターンC: ローカル開発 / テスト環境
メール送信設定を行わない場合、認証コードは **サーバーのコンソール（ターミナル画面）** に自動で表示されます。テストや開発で手軽にログインを試したいときに便利です。
...
> **注意**: パスキー認証はブラウザのセキュリティ仕様上、**HTTPS環境**（またはローカルの `localhost`）でのみ動作します。
