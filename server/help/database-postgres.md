...
既存DBへ反映する場合は、すでに適用済みのファイルを記録し、未適用分だけを番号順に実行してください。現在のディレクトリには初期スキーマに加えて、プロフィール、通知、Push、モデレーション、投稿公開範囲、ログイン保護、グループDM、DM公開鍵、監査ログに関する追加マイグレーションがあります。
...
ステージングではログイン、投稿、検索、添付アップロード、通知、DM、Push購読を確認します。非公開投稿、検索除外、ブロック関係についても、取得経路にかかわらず同じ可視性ルールになることを確認してください。
