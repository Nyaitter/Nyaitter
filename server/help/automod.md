...
OpenAI、Groq、OpenRouter、ローカル LLM（Ollama, vLLM 等）を使う場合の設定です。
...
サーバーのルートまたは `server/` にルールファイル（`rule.nd` または `RULES_FILE_PATH` で指定したファイル）が置かれている場合、AutoMod はそのルール全文を自動で読み込み、コミュニティの規約に沿って判定を行います。
...
|---|---|
| `<safe>` (安全) | 通常通り公開されます。 |
| `<low>` (軽度) | 投稿に**ワンクッション**（閲覧注意）が付与されます。 |
