...
const config = {
  ...
  database: {
    ...
    cockroach: {
      ...
      poolMin: Math.min(20, Math.max(1, Math.floor(Number(process.env.COCKROACH_POOL_MIN || get('database.cockroach.poolMin', 2)) || 2))),
      poolIdleTimeoutMs: Math.min(86400000, Math.max(1000, Math.floor(Number(process.env.COCKROACH_POOL_IDLE_TIMEOUT_MS || get('database.cockroach.poolIdleTimeoutMs', 300000)) || 300000))),
      connectionTimeoutMs: Math.min(60000, Math.max(1000, Math.floor(Number(process.env.COCKROACH_CONNECTION_TIMEOUT_MS || get('database.cockroach.connectionTimeoutMs', 15000)) || 15000))),
