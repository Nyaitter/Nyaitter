...
async function getRunningStatus() {
  try {
    const response = await requestOperatorCommand({ action: 'status' }, { timeoutMs: 500 });
...
async function stopServer() {
  ...
  if (!response?.ok) throw new Error(response?.error || 'サーバー停止要求に失敗しました');
...
async function setAdministrator(userIdArgument, admin) {
  ...
  if (!response?.ok) throw new Error(response?.error || '管理者権限の更新に失敗しました');
  console.log(`#${response.user.id} の管理者権限を${response.user.admin ? '付与' : '解除'}しました。`);
}
...
async function main(argv) {
  ...
  if (group === 'server') {
    ...
    if (command === 'status') {
      const status = await getRunningStatus();
      if (!status) {
        console.log(`停止中（制御ソケット: ${getOperatorSocketPath()}）`);
        process.exitCode = 1;
        return;
      }
      console.log(formatStatus(status));
