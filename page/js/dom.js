export const DOM = {
	...
	window.addEventListener('unhandledrejection', (event) => {
		const reason = event.reason instanceof Error ? event.reason.message : String(event.reason || '不明なエラー');
	...
	document.getElementById('mainjs-error-reload-btn')?.addEventListener('click', () => window.location.reload());
	DOM.imagePreviewModal?.addEventListener('click', (event) => {
		if (event.target === DOM.imagePreviewModal || event.target.closest('.modal-close-btn')) {
