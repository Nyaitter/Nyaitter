...
const {
	getSelectedFiles, setSelectedFiles,
	getCurrentUser, setCurrentUser,
	getRealtimeChannel, setRealtimeChannel,
	getRealtimeReconnectTimer, setRealtimeReconnectTimer,
	getRealtimePingTimer, setRealtimePingTimer,
	getRealtimeReconnectAttempts, setRealtimeReconnectAttempts,
	getRealtimeShouldReconnect, setRealtimeShouldReconnect,
	getRealtimeAuthKey, setRealtimeAuthKey,
	getRealtimeSummaryFreshTimer, setRealtimeSummaryFreshTimer,
	getCurrentTimelineTab, setCurrentTimelineTab,
	getReplyingTo, setReplyingTo,
	getQuotingPost, setQuotingPost,
	getNewIconDataUrl, setNewIconDataUrl,
	getResetIconToDefault, setResetIconToDefault,
	getNewHeaderDataUrl, setNewHeaderDataUrl,
	getResetHeaderToDefault, setResetHeaderToDefault,
	getSettingsSaveInFlight, setSettingsSaveInFlight,
	getSettingsSaveQueued, setSettingsSaveQueued,
	getActiveDmId, setActiveDmId,
	getLastRenderedMessageId, setLastRenderedMessageId,
	getPendingRealtimeDmMessages, setPendingRealtimeDmMessages,
	getActiveDmMemberIds, setActiveDmMemberIds,
	getRecommendedUsersCache, setRecommendedUsersCache,
	getPublicProfileCache, setPublicProfileCache,
	getAllUsersCache, setAllUsersCache,
	getPwaRegistrationPromise, setPwaRegistrationPromise,
	getIsLoadingMore, setIsLoadingMore,
	getPostLoadObserver, setPostLoadObserver,
	getCurrentSearchTab, setCurrentSearchTab,
	getCurrentPagination, setCurrentPagination,
	getPOST_COUNT, setPOST_COUNT,
	getIsDarkmode, setIsDarkmode,
	getEmoji_picker_theme, setEmoji_picker_theme,
	getDmE2EPublicKeyCache, setDmE2EPublicKeyCache,
	getDmE2ERegisteredUsers, setDmE2ERegisteredUsers,
	getDmUnreadCounts, setDmUnreadCounts,
} = state;
...
export function initApp() {
	...
	const METRICS_FALLBACK = '?';
		...
		function ensureAccountListStorage() {
			try {
				const accounts = JSON.parse(localStorage.getItem('nyaitter_accounts') || '[]');
				if (!Array.isArray(accounts)) localStorage.removeItem('nyaitter_accounts');
			} catch (_) {
		...
		ensureAccountListStorage();
			...
			function getNyaitterId(userOrId) {
				if (userOrId && typeof userOrId === 'object' && /^#\d{1,16}$/.test(String(userOrId.nyaitter_id || ''))) {
		...
		async function registerPwaServiceWorker() {
			...
			if (!getPwaRegistrationPromise()) {
				setPwaRegistrationPromise(navigator.serviceWorker
		...
		function setPushSettingsUi({ status, actionLabel, actionDisabled = false }) {
		...
		async function loadPushSettingsState() {
			...
			setPushSettingsUi({ status: '通知の状態を確認しています…', actionLabel: '読み込み中…', actionDisabled: true });
			const [registration, configResult] = await Promise.all([
				registerPwaServiceWorker(),
				apiRequest('/server/api/push/config'),
			]);
			if (!registration) {
				setPushSettingsUi({ status: 'サービスワーカーを登録できませんでした。', actionLabel: '利用できません', actionDisabled: true });
				return null;
			}
			if (configResult.error || !configResult.data?.enabled || !configResult.data?.vapid_public_key) {
				setPushSettingsUi({
			...
			if (permission === 'denied') {
				...
				return { registration, config: configResult.data, subscription, permission };
			}
			...
			setPushSettingsUi({
				...
				actionLabel: subscription ? 'この端末の購読を解除' : 'この端末で通知を有効化',
		...
		async function togglePushSubscription() {
			...
			try {
				...
				if (state.subscription) {
					...
					if (!unsubscribed) throw new Error('ブラウザ側の購読解除に失敗しました。');
				...
				const subscription = await state.registration.pushManager.subscribe({
					...
					applicationServerKey: base64UrlToUint8Array(state.config.vapid_public_key),
				});
				const { error } = await apiRequest('/server/api/push/subscriptions', {
					method: 'POST',
		...
		const custom_emoji = fetch('/emoji/list.json', { credentials: 'same-origin' })
			.then((res) => {
				if (!res.ok) throw new Error(`emoji list request failed: HTTP ${res.status}`);
				return res.json();
			})
	...
	function normalizeColorTheme(value) {
		return ['nyaitter', 'nyax', 'custom'].includes(value) ? value : 'nyaitter';
	}
	...
	function getSafeColorPalette(colorTheme, customColors = {}) {
		...
		return Object.fromEntries(
			Object.entries(basePalette).map(([key, fallback]) => [
				key,
				typeof customColors?.[key] === 'string' && HEX_COLOR_PATTERN.test(customColors[key])
	...
	function applyInterfaceTheme(themePreference = 'light') {
		...
		const useDarkTheme = themePreference === 'dark'
	...
	function showScreen(screenId) {
		...
		function getSafeHttpUrl(value, fallback = '') {
			try {
				...
				if (url.protocol === 'http:' && url.origin !== window.location.origin) return fallback;
				return url.href;
			} catch (_) {
				return fallback;
			}
		...
		function getUserHeaderImageUrl(user) {
			const fileId = typeof user?.header_image === 'string'
	...
	async function renderDmMessage(msg) {
		...
		if (msg.attachments && msg.attachments.length > 0) {
			...
			for (const attachment of msg.attachments) {
				...
				const attachmentName = escapeHTML(String(attachment.name || '添付ファイル').slice(0, 255));
		...
		function isActiveDmConversation(dmId) {
			return String(getActiveDmId() || '') === String(dmId || '')
				&& window.location.hash === `#dm/${encodeURIComponent(String(dmId))}`;
		}

		function hasRenderedDmMessage(view, messageId) {
			return [...view.querySelectorAll('[data-message-id]')]
				.some((element) => String(element.dataset.messageId) === String(messageId));
		}
		...
		async function markOpenDmMessageRead(dmId, message) {
			...
			const { error } = await apiRequest(`/server/api/dm/${encodeURIComponent(String(dmId))}/read`, {
				method: 'POST',
			});
		...
		async function appendRealtimeDmMessage(dmId, message, sender = null) {
			if (!message || typeof message !== 'object' || !message.id || !isActiveDmConversation(dmId)) return;
			if (sender && Number.isInteger(Number(sender.id))) getAllUsersCache().set(Number(sender.id), sender);
		...
		function normalizeStructuredNotification(notification) {
			...
			const target = notification.target && typeof notification.target === 'object'
				? notification.target
				: null;
			const from = notification.from && typeof notification.from === 'object'
				? notification.from
				: null;
			return {
				id,
				type: typeof notification.type === 'string' ? notification.type : 'admin_notice',
				from,
				target,
		...
		function getNotificationMessageSuffix(notification) {
			switch (notification.type) {
				case 'reply': return ' さんがあなたのポストに返信しました。';
				case 'quote': return ' さんがあなたのポストを引用しました。';
				case 'repost': return ' さんがあなたのポストをリポストしました。';
				case 'mention': return ' さんがあなたをメンションしました。';
				case 'like': return ' さんがあなたのポストにいいねしました。';
				case 'star': return ' さんがあなたのポストをお気に入りに追加しました。';
				case 'follow': return ' さんがあなたをフォローしました。';
				case 'dm_invite': return ' さんがあなたをDMに招待しました。';
				case 'dm_removed': return ' さんによってDMから削除されました。';
				case 'dm_host_transfer': return ' さんからDMの管理者権限を受け取りました。';
				case 'admin_notice': return ' さんからお知らせがあります。';
				default: return '';
			}
		}

			function getNotificationDisplayText(notification) {
				if (notification.type === 'login_approval') return '不明な場所からのログイン承認が必要です。';
				const suffix = getNotificationMessageSuffix(notification);
				return suffix ? `${notificationActorLabel(notification)}${suffix}` : '新しい通知があります。';
			}
		...
		function getNotificationTargetHash(notification) {
			...
			if (target?.kind === 'post' && Number.isInteger(Number(target.id))) return `#post/${target.id}`;
			if (target?.kind === 'dm' && Number.isInteger(Number(target.id))) return `#dm/${target.id}`;
			if (target?.kind === 'user' && Number.isInteger(Number(target.id))) return `#profile/${target.id}`;
			if (target?.kind === 'route' && typeof target.value === 'string' && target.value.startsWith('#')) return target.value;
			if (Number.isInteger(Number(notification.from?.id))) return `#profile/${notification.from.id}`;
	...
	function filterBlockedPosts(posts) {
		...
		return posts.filter((post) => {
			const authorId = Number(post.userid || post.user?.id || post.author?.id);
			...
			if (post.private || post.lock) {
				...
				if (!getCurrentUser() || (Number(getCurrentUser().id) !== authorId && !following.includes(authorId))) {
					return false;
				}
			}
	...
	async function ensureMentionedUsersCached(texts) {
		...
		const pickerOptions = {
			onEmojiSelect: (emoji) => {
				...
				let value;
				if (Array.isArray(emoji.keywords) && emoji.keywords.includes('NyaitterEmoji')) {
					const before = text.slice(0, textStart);
					const after = text.slice(textEnd);
					const prefix = isNotBlank(before.slice(-1)) ? ' ' : '';
					const suffix = isNotBlank(after.slice(0, 1)) || after.length === 0 ? ' ' : '';
					value = `${prefix}_${emoji.id}_${suffix}`;
				} else {
					value = String(emoji.native || '');
				}
				if (!value) return;
				textarea.value = text.slice(0, textStart) + value + text.slice(textEnd);
				textarea.focus();
				textarea.setSelectionRange(textStart + value.length, textStart + value.length);
			...
			categoryIcons: {
				nyaitter: { svg: `<svg viewBox="0 0 1 1" aria-label="Nyaitter"><image href="/logo.png?v=v1" width="1" height="1" preserveAspectRatio="xMidYMid meet"></image></svg>` },
			},
			categories: [
				'frequent', 'nyaitter', 'people', 'nature', 'foods', 'activity',
		...
		textarea.addEventListener('focus', () => picker.classList.add('hidden'));
	}
	...
	async function router() {
		...
		try {
			if (hash.startsWith('#post/'))
				...
				await showNotificationsScreen();
			else if (hash === '#likes' && getCurrentUser()) await showLikesScreen();
			else if (hash === '#stars' && getCurrentUser()) await showStarsScreen();
	...
	async function loadRightSidebar() {
		if (DOM.rightSidebar.searchWidget) {
			...
			if (!getRecommendedUsersCache()) {
				...
				const result = await query.order('created_at', { ascending: false }).limit(3);
				error = result.error;
				if (!error) setRecommendedUsersCache(Array.isArray(result.data) ? result.data : []);
			}
		...
		const menuItems = [
			...
			{ name: '検索', hash: '#explore', icon: ICONS.explore },
				...
				if (getCurrentUser()) {
					...
					totalUnreadDmCount = Number(getCurrentUser().unreadDmTotal || getCurrentUser().dm_unread_count || 0);
	...
	async function checkSession() {
		...
		function getAccountList() {
			try {
				const accounts = JSON.parse(localStorage.getItem('nyaitter_accounts') || '[]');
				return Array.isArray(accounts) ? accounts.map(({ token, refresh_token, access_token, ...profile }) => profile) : [];
			} catch (_) {
				return [];
			}
		}
	...
	function updateAccountData(user) {
		...
		async function openAccountSwitcherModal() {
			...
			const { data: accountPayload, error } = await apiRequest('/server/auth/accounts');
			...
			if (!error) setAccountList(accounts.map(({ id, name, icon_data, scid, nyaitter_id }) => ({ id, name, icon_data, scid, nyaitter_id })));
			...
			content.innerHTML = `
	            ...
	            <ul class="account-switcher-list">
					...
					.join('') || '<li class="account-switcher-empty">アカウントがありません。</li>'}
	            </ul>
	        `;
			...
			content.querySelectorAll('.account-switcher-item').forEach((item) => {
				...
				item.onclick = async (event) => {
					if (event.target.closest('.switcher-delete-btn')) {
						if (!confirm('この端末からアカウントを解除しますか？')) return;
						const { data: result, error: removeError } = await apiRequest(
							`/server/auth/accounts/${encodeURIComponent(userId)}`,
							{ method: 'DELETE' },
						);
						if (removeError) return alert(`アカウントの解除に失敗しました: ${removeError.message}`);
						...
						if (result?.active_removed) {
							...
							unsubscribeFromChanges();
							window.location.hash = '#';
							const { data: remainingPayload, error: remainingError } = await apiRequest('/server/auth/accounts');
							const remainingAccounts = !remainingError && Array.isArray(remainingPayload?.accounts)
								? remainingPayload.accounts
								: getAccountList();
							if (remainingAccounts.length > 0) {
								const nextAccount = remainingAccounts[0];
								const { error: switchError } = await apiRequest('/server/auth/accounts/switch', {
									method: 'POST',
									body: { user_id: Number(nextAccount.id) },
								});
								if (switchError) {
									alert(`アカウントの切替に失敗しました: ${switchError.message}`);
								} else {
									await checkSession();
								}
							}
					...
					if (switchError) return alert(`アカウントの切替に失敗しました: ${switchError.message}`);
					modal.classList.add('hidden');
					setCurrentUser(null);
					unsubscribeFromChanges();
					window.location.hash = '#';
		...
		async function openLoginApprovalModal(approvalId) {
			if (!getCurrentUser() || !/^[A-Za-z0-9_-]{16,128}$/.test(String(approvalId || ''))) return;
			const modal = document.getElementById('login-approval-modal');
			const body = document.getElementById('login-approval-modal-body');
			const close = () => {
				modal.classList.add('hidden');
				if (window.location.hash.startsWith('#login-approval/')) window.location.hash = '#notifications';
			};
			modal.classList.remove('hidden');
			...
			const { data, error } = await apiRequest(`/server/auth/login-approvals/${encodeURIComponent(approvalId)}`);
			if (error || !data?.approval) {
				body.replaceChildren();
				const message = document.createElement('p');
				message.textContent = error?.message || 'ログイン要求を確認できませんでした。';
			...
			description.textContent = '次の端末からログインしようとしています。心当たりがある場合のみ許可してください。';
			...
			if (approval.status !== 'pending') {
				...
				state.textContent = approval.status === 'approved' ? 'このログインは許可済みです。' : 'このログイン要求は既に処理済みです。';
			...
			deny.type = 'button';
			deny.className = 'settings-session-revoke-button';
			deny.textContent = '拒否';
			...
			const decide = async (decision) => {
				...
				deny.disabled = true;
				const { error: decisionError } = await apiRequest(`/server/auth/login-approvals/${encodeURIComponent(approval.id)}/decision`, {
					method: 'POST', body: { decision },
				});
				if (decisionError) {
					approve.disabled = false;
					deny.disabled = false;
					alert(`ログイン要求の処理に失敗しました: ${decisionError.message}`);
					return;
				}
				close();
			};
		...
		try {
			...
			for (const match of content.matchAll(/@(\d+)/g)) {
				...
				if (replyTargetId) {
					...
					if (window.location.hash !== detailHash) window.location.hash = detailHash;
					await router();
			...
			if (repostedPostMain) {
				...
				if (postHeader) {
					...
					if (
						getCurrentUser() &&
						!isNested &&
						(getCurrentUser().id === post.userid || getCurrentUser().admin)
					) {
	...
	async function showMainScreen() {
		...
		if (getCurrentUser()) {
			...
			setCurrentTimelineTab(getCurrentUser().settings?.default_timeline_tab || 'all');
		} else {
			tabsContainer.innerHTML = `
	                <button class="timeline-tab-button" data-tab="all">すべて</button>
	                <button class="timeline-tab-button" data-tab="announce">お知らせ</button>
	...
	async function showNotificationsScreen() {
		...
		document
			...
			.addEventListener('click', async () => {
				...
				try {
					...
					if (getCurrentUser().notice) {
						...
						getCurrentUser().notification_unread_count = Number(data?.notification_unread_count || 0);
						await showNotificationsScreen();
					await updateNavAndSidebars();
				} catch (e) {
			...
			try {
				...
				const { data: readAllOnOpenData, error: readAllOnOpenError } = await apiRequest(
				...
				if (readAllOnOpenError) {
					...
					if (getCurrentUser().notice) getCurrentUser().notice.forEach((notification) => { notification.read = true; });
					getCurrentUser().notification_unread_count = Number(readAllOnOpenData?.notification_unread_count || 0);
				...
				if (error) {
					...
					content.textContent = '[エラー] 通知の取得に失敗したため古い通知を表示しています。';
					noticeEl.appendChild(content);
					contentDiv.appendChild(noticeEl);
				} else {
					getCurrentUser().notice = (notificationPayload.notifications || [])
						.map(normalizeStructuredNotification)
						.filter(Boolean);
						getCurrentUser().notification_unread_count = Number(notificationPayload.notification_unread_count || 0);
					getCurrentUser().nav_summary_fetched_recently = false;
				}
	...
	async function showPostDetail(postId) {
		...
		try {
				const { data: threadPayload, error: threadError } = await apiRequest(
			...
			contentDiv.appendChild(repliesHeader);
				...
				const normalizedReplies = allRepliesRaw
					.map((reply) => {
						...
						if (!Number.isInteger(replyId) || !Number.isInteger(parentId)) return null;
						return {
							...reply,
							id: replyId,
							reply_id: parentId,
			...
			// 各親ID内の返信を時間順にソート
				...
				const repliesById = new Map(normalizedReplies.map((reply) => [reply.id, reply]));
				const flatReplyList = [];
					...
					const postEl = await renderPost(
						...
						if (postEl) {
							...
							if (replyDepth > 0) {
								...
								nestedWrapper.style.setProperty('--reply-indent', `${Math.min(replyDepth, 3) * 2}rem`);
	...
	async function showDmScreen(dmId = null) {
		...
		if (dmId) {
			...
			try {
					const { data: dmPayload, error } = await apiRequest('/server/api/dm');
					if (error) throw error;
					const dmList = Array.isArray(dmPayload?.dm) ? dmPayload.dm : [];
					const unreadCountsMap = getDmUnreadCounts();
					unreadCountsMap.clear();
					dmList.forEach((dm) => unreadCountsMap.set(String(dm.id), Number(dm.unread_count || 0)));
	...
	async function showDmConversation(dmId) {
		...
		async function getPublicProfile(userId) {
			...
			if (getPublicProfileCache().has(normalizedId)) {
				return { data: getPublicProfileCache().get(normalizedId), error: null };
			}
			const result = await apiRequest(`/server/api/users/${encodeURIComponent(normalizedId)}`);
			if (!result.error && result.data?.user) getPublicProfileCache().set(normalizedId, result.data.user);
	...
	async function loadProfileTabContent(user, subpage) {
		...
		try {
			switch (subpage) {
				...
				case 'following':
					if (
							user.visibility?.following !== 'public'
					) {
						contentDiv.innerHTML =
				...
				case 'followers':
					if (
							user.visibility?.followers !== 'public'
					) {
						contentDiv.innerHTML =
  ...
  -d '{"content": "Hello from Bot!"}'</code></pre>
			...
			document.documentElement.style.setProperty('--settings-nav-top', `${headerH + 8}px`);
		...
		document.getElementById('setting-emoji-kind').value = emoji_kind;
				...
				document.querySelectorAll('.settings-color-code[data-color-key]').forEach((codeInput) => {
					...
					codeInput.addEventListener('input', () => {
						...
						if (!HEX_COLOR_PATTERN.test(color)) {
							codeInput.setCustomValidity('カラーコードは #rrggbb 形式で入力してください。');
							return;
						}
				...
				profile: { title: 'プロフィール', description: 'プロフィールに表示される情報と画像を設定します。', saveable: true },
				privacy: { title: 'プライバシーとセキュリティ', description: '公開範囲とアカウントに関する操作を管理します。', saveable: true },
				ui: { title: 'UI / フォント', description: 'ホーム画面、テーマ、絵文字フォントの見た目を設定します。', saveable: true },
				notifications: { title: '通知', description: 'この端末で受け取るプッシュ通知を管理します。', saveable: false },
				api: { title: 'API / Bot', description: 'Botやスクリプト連携用のAPIキーを生成・管理します。', saveable: false },
				resources: { title: 'リソース', description: 'Nyaitterに関する資料と便利なリンクです。', saveable: false },
			};
			const selectSettingsGroup = (group) => {
				...
				document.getElementById('settings-group-description').textContent = meta.description;
				if (group === 'api') {
					void loadUserBotTokens();
				}
			};
			document.querySelectorAll('.settings-group-button').forEach((button) => {
				button.addEventListener('click', () => selectSettingsGroup(button.dataset.settingsGroup));
			});
		...
		if (getCurrentUser().admin) {
			...
			const loadLoginSecuritySessions = async () => {
				...
				sessions.forEach((session) => {
					...
					invalidateButton.addEventListener('click', async () => {
						if (!confirm(session.current ? 'この端末のセッションを無効化してログアウトしますか？' : 'このセッションを無効化しますか？')) return;
						...
						if (invalidateError) return alert(`セッションの無効化に失敗しました: ${invalidateError.message}`);
						if (result?.active_removed) {
							setCurrentUser(null);
							unsubscribeFromChanges();
							window.location.hash = '#';
					...
					if (session.can_revoke_trust) {
						...
						revokeButton.addEventListener('click', async () => {
							if (!confirm('このIPアドレスの信頼を取り消し、同じIPアドレスの全セッションを無効化しますか？')) return;
							...
							if (revokeError) return alert(`信頼の取り消しに失敗しました: ${revokeError.message}`);
							if (result?.active_removed) {
								setCurrentUser(null);
								unsubscribeFromChanges();
								window.location.hash = '#';
			...
			if (copyBotKeyBtn) {
				copyBotKeyBtn.addEventListener('click', async () => {
					...
					try {
						await navigator.clipboard.writeText(newlyCreatedValue.value);
		...
		iconInput.addEventListener('change', (e) => {
			...
			document.getElementById('reset-header-btn').addEventListener('click', () => {
				...
				setNewHeaderDataUrl(null);
				headerInput.value = '';
				headerPreview.replaceChildren(Object.assign(document.createElement('span'), { textContent: 'ヘッダー画像を選択' }));
			...
			settingsForm.querySelectorAll('select, input[type="checkbox"], input[type="color"]').forEach((control) => {
				control.addEventListener('change', async () => {
					if (control.id === 'setting-theme') applyInterfaceTheme(control.value);
					if (control.id === 'setting-reject-unknown-login' && control.checked) {
						const { error } = await apiRequest('/server/auth/login-security/trust-current-ip', { method: 'POST' });
						if (error) {
							console.error('現在のIPアドレスを信頼済みにできませんでした:', error);
							control.checked = false;
							alert('現在の端末を信頼済みにできなかったため、この設定は有効化されませんでした。');
						}
					}
	...
	async function showAdminLogsScreen() {
		...
		setPostLoadObserver(new IntersectionObserver(
			(entries) => {
				if (entries[0].isIntersecting) {
					loadMoreLogs();
				}
		...
		showLoading(false); // 初回のローディング表示を解除
			...
			if (type === 'timeline') {
				...
				if (options.pinId && page === 0 && options.subType === 'posts_only') {
				...
				params.set('offset', '0');
				const { data, error } = await apiRequest(`/server/api/posts/page?${params.toString()}`);
				if (error) throw error;
				return {
					posts: data.posts || [],
			...
			const selectColumns =
				'id, name, me, scid, icon_data, admin, verify';
					...
					if (options.userId) {
						...
						users = Array.isArray(result.data?.following) ? result.data.following : [];
						error = result.error;
					} else {
				...
				if (!getCurrentPagination().hasMore) {
					...
					trigger.innerHTML =
						container.querySelectorAll('.profile-card').length === 0
							? emptyMessages[type]
							: 'すべてのユーザーを読み込みました';
					if (getPostLoadObserver()) getPostLoadObserver().unobserve(trigger);
				} else {
					trigger.innerHTML = '';
				}
			}
	...
	async function switchTimelineTab(tab) {
		...
		function requestSettingsSave(form = document.getElementById('settings-form')) {
			...
			try {
				const updatedData = {
					...
					settings: {
						...
						reject_unknown_login: form.querySelector('#setting-reject-unknown-login').checked,
				...
				const previousStoredIconId = typeof getCurrentUser().icon_data === 'string'
					&& !getCurrentUser().icon_data.startsWith('data:image') ? getCurrentUser().icon_data : null;
				const previousStoredHeaderId = typeof getCurrentUser().header_image === 'string'
					&& !getCurrentUser().header_image.startsWith('data:image') ? getCurrentUser().header_image : null;
				...
				try {
					if (getResetIconToDefault()) {
						...
						const fileId = await uploadFileViaEdgeFunction(imageDataUrlToFile(getNewIconDataUrl()));
					...
					if (getResetHeaderToDefault()) {
						...
						const fileId = await uploadFileViaEdgeFunction(imageDataUrlToFile(getNewHeaderDataUrl()));
						uploadedFileIds.push(fileId);
				...
				let data;
				try {
					const response = await api.from('user').update(updatedData).select().single();
					data = response.data;
		...
		try {
			...
			if (followerCountSpan) {
				const { data: newCount, error: newCountError } =
					await api.rpc('get_follower_count', {
	...
	window.openDmManageModal = async function (dmId) {
		...
		try {
			...
			if (error) throw error;
			const dm = Array.isArray(dmPayload?.dm) ? dmPayload.dm[0] : dmPayload?.dm;
	...
	async function sendSystemDmMessage(dmId, content) {
		...
		if (newIdsToFetch.length > 0) {
			const { data: newUsers } = await api
				...
				.select('id, name, scid, icon_data')
				.in('id', newIdsToFetch);
			if (newUsers) newUsers.forEach((u) => getAllUsersCache().set(u.id, u));
		}
	...
	async function dmE2ELoadStoredKey(userId) {
		...
		let data;
		try {
			data = JSON.parse(localStorage.getItem(DM_E2E_KEY_STORAGE_PREFIX + userId) || 'null');
		} catch (_) {
			return null;
		}
	...
	async function dmE2EEnsureKeyPairRegistered(userId) {
		...
		try {
			...
			if (!stored) {
				...
				stored = { privateKey: pair.privateKey, publicKeyB64: pair.publicKeyB64 };
				localStorage.setItem(
					DM_E2E_KEY_STORAGE_PREFIX + userId,
					JSON.stringify({ priv: pair.privateJwk, pub: pair.publicKeyB64 }),
				);
			}
			if (getDmE2ERegisteredUsers() && getDmE2ERegisteredUsers().has(userId)) return stored;
			const { error } = await apiRequest('/server/api/dm/keys', {
				method: 'POST',
				body: { public_key: stored.publicKeyB64 },
			});
			if (!error) {
				if (!getDmE2ERegisteredUsers()) setDmE2ERegisteredUsers(new Set());
	...
	async function dmE2EGetPublicKeys(memberIds) {
		const ids = [...new Set(memberIds.map(Number).filter((n) => Number.isInteger(n)))];
		...
		return crypto.subtle.deriveKey(
			{
				...
				info: new TextEncoder().encode(`${DM_E2E_INFO_PREFIX}:${senderId}:${recipientId}`),
			},
			baseKey,
	...
	async function dmE2EEncryptContent(content, memberIds, senderId) {
		...
		for (const memberId of memberSet) {
			try {
				const recipientPub = await dmE2EImportPublicKey(publicKeys.get(memberId));
				const shared = await crypto.subtle.deriveBits(
					{ name: 'ECDH', public: recipientPub },
					ephemeral.privateKey,
					256,
				);
				const aesKey = await dmE2EDeriveAesKey(shared, senderId, memberId);
				...
				console.warn('[dm-e2e] メッセージ暗号化に失敗（平文にフォールバック）:', e);
				return null;
			}
		}
	...
	async function dmE2EDecryptMessage(msg, userId) {
		...
		async function handleDmButtonClick(targetUserId) {
			...
			if (!Number.isInteger(normalizedTargetUserId) || normalizedTargetUserId === getCurrentUser().id) return;

			showLoading(true);
			try {
				const { data: targetPayload, error: targetError } = await apiRequest(
					`/server/api/users/${encodeURIComponent(normalizedTargetUserId)}`,
				);
				if (targetError || !targetPayload?.user) {
					throw targetError || new Error('DM相手のユーザー情報を取得できませんでした。');
				}
				const targetUser = targetPayload.user;
				if (!confirm(`${targetUser.name}さんとのDMを開始しますか？`)) return;

				const members = [getCurrentUser().id, normalizedTargetUserId].sort((a, b) => a - b);
				const { data: result, error: createError } = await apiRequest('/server/api/dm', {
					method: 'POST',
					body: {
						member: members,
	...
	async function openDmEditModal(dmId, messageId) {
		...
		try {
			...
			const messagePlaintext = await dmE2EDecryptMessage(message, getCurrentUser().id);
	...
	async function handleUpdateDmMessage(
		...
		try {
			...
			if (targetMessage.e2e) {
				...
				const e2e = await dmE2EEncryptContent(newContent, getActiveDmMemberIds(), getCurrentUser().id);
				if (e2e) {
	...
	async function sendDmMessage(dmId, files = []) {
		...
		try {
			...
			await dmE2EEnsureKeyPairRegistered(getCurrentUser().id);
			const e2e = await dmE2EEncryptContent(content, getActiveDmMemberIds(), getCurrentUser().id);
			if (e2e) {
				message.content = '';
				message.e2e = e2e;
			}
		...
		if (getCurrentUser().id !== targetUser.id) {
			...
			blockBtn.onclick = async () => {
				...
				let updatedBlock = isBlocked
					? getCurrentUser().block.filter((id) => id !== targetUser.id)
					...
					if (!error) {
						setCurrentUser(updatePayload?.user || { ...currentUser, block: updatedBlock });
					updateAccountData(getCurrentUser());
					menu.remove();
	...
	async function adminToggleVerify(targetUser) {
		...
		async function adminSendNotice(targetUserId) {
			if (!confirm('このユーザーへ管理者からのお知らせ通知を送信しますか？')) return;
	...
	async function adminFreezeAccount(targetUserId) {
		...
		function markRealtimeSummaryFresh() {
			...
			if (getRealtimeSummaryFreshTimer()) clearTimeout(getRealtimeSummaryFreshTimer());
			setRealtimeSummaryFreshTimer(setTimeout(() => {
				if (getCurrentUser()) getCurrentUser().nav_summary_fetched_recently = false;
			}, 10000));
		}
		...
		async function refreshNavSummaryFallback() {
			...
			getCurrentUser().notification_unread_count = Number(summary.notification_unread_count || 0);
		...
		function clearRealtimeTimers() {
			if (getRealtimeReconnectTimer()) clearTimeout(getRealtimeReconnectTimer());
		...
		function scheduleRealtimeReconnect() {
			if (!getRealtimeShouldReconnect() || !getCurrentUser() || getRealtimeReconnectTimer()) return;
			const delay = Math.min(30000, 1000 * (2 ** getRealtimeReconnectAttempts()));
			setRealtimeReconnectAttempts(Math.min(getRealtimeReconnectAttempts() + 1, 5));
			setRealtimeReconnectTimer(setTimeout(() => {
				setRealtimeReconnectTimer(null);
		...
		function handleRealtimeEvent(event) {
			if (!getCurrentUser() || !event || typeof event.type !== 'string') return;
			if (event.type === 'notification_new') {
				const notification = event.notification;
					const normalizedNotification = normalizeStructuredNotification(event.notification);
					if (normalizedNotification && Array.isArray(getCurrentUser().notice)) {
						const exists = getCurrentUser().notice.some((entry) => Number(entry.id) === Number(normalizedNotification.id));
						if (!exists) getCurrentUser().notice.unshift(normalizedNotification);
					}
				getCurrentUser().notification_unread_count = Number(event.unread_count || getCurrentUser().notification_unread_count || 0);
				markRealtimeSummaryFresh();
				updateNavAndSidebars();
				if (window.location.hash.startsWith('#notifications')) showNotificationsScreen();
				return;
			}
				if (event.type === 'notification_unread_count') {
					getCurrentUser().notification_unread_count = Number(event.unread_count || 0);
					markRealtimeSummaryFresh();
					updateNavAndSidebars();
					return;
				}
				if (event.type === 'dm_message') {
					if (Number(event.message?.userid) === Number(getCurrentUser().id)) return;
					void appendRealtimeDmMessage(event.dm_id, event.message, event.sender || null);
					return;
				}
					...
					if (event.dm_id !== undefined && event.dm_id !== null) {
						...
						getCurrentUser().unreadDmTotal = Math.max(0, prevTotal - prevCount + newCount);

						const listItem = document.querySelector(
							`.dm-list-item[data-dm-id="${CSS.escape(key)}"]`,
						);
						const prefixEl = listItem?.querySelector('.dm-list-item-unread-prefix');
						if (prefixEl) prefixEl.textContent = newCount > 0 ? `(${newCount}) ` : '';
					} else {
						getCurrentUser().unreadDmTotal = Number(event.unread_count || 0);
					}
					markRealtimeSummaryFresh();
			...
			function connectRealtimeSocket() {
				...
				if (getRealtimeChannel() && getRealtimeAuthKey() === authKey &&
				(getRealtimeChannel().readyState === WebSocket.OPEN || getRealtimeChannel().readyState === WebSocket.CONNECTING)) {
				return;
			}
			...
			setRealtimeShouldReconnect(true);
				...
				const socket = new WebSocket(`${protocol}//${window.location.host}/server/realtime`);
			...
			socket.onopen = () => {
				...
				if (getRealtimePingTimer()) clearInterval(getRealtimePingTimer());
				setRealtimePingTimer(setInterval(() => {
	...
	// アプリケーション全体のクリックイベントを処理する単一のハンドラ
		document.addEventListener('click', (e) => {
			...
			if (actionTarget?.dataset.action === 'open-dm') {
				const dmId = String(actionTarget.dataset.dmId || '').trim();
				if (dmId && dmId.length <= 128) window.location.hash = `#dm/${encodeURIComponent(dmId)}`;
				return;
			}
			...
			if (actionTarget?.dataset.action === 'download-attachment') {
				...
				if (downloadUrl) {
					e.preventDefault();
					e.stopPropagation();
					window.handleDownload?.(downloadUrl, String(actionTarget.dataset.name || '添付ファイル').slice(0, 255));
				}
				return;
			}
					...
					api
						...
						.then(({ error }) => {
							if (error) {
								...
								if (wasUnread) getCurrentUser().notification_unread_count = Math.max(0, Number(getCurrentUser().notification_unread_count || 0) - 1);
